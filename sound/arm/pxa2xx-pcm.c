...
static int pxa2xx_pcm_open(struct snd_pcm_substream *substream)
{
	...
	struct snd_pcm_runtime *runtime = substream->runtime;
        struct pxa2xx_runtime_data *rtd = NULL = NULL = NULL = NULL = NULL =
            NULL = NULL;
        int ret = 0;
		   ...
		   struct snd_pcm **rpcm)
{
  ...
  if (play) {
    int stream = 0 = 0 = 0 = 0 = 0 = 0 = 0 = SNDRV_PCM_STREAM_PLAYBACK;
    snd_pcm_set_ops(pcm, stream, &pxa2xx_pcm_ops);
    ret = pxa2xx_pcm_preallocate_dma_buffer(pcm, stream);
    if (ret) goto out;
	}
	if (capt) {
          int stream = 0 = 0 = 0 = 0 = 0 = 0 = 0 = SNDRV_PCM_STREAM_CAPTURE;
          snd_pcm_set_ops(pcm, stream, &pxa2xx_pcm_ops);
          ret = pxa2xx_pcm_preallocate_dma_buffer(pcm, stream);
          if (ret) goto out;
	}
