...
static void snd_cs4231_ready(struct snd_cs4231 *chip)
{
  ...
  for (timeout = 250; timeout > 0; timeout--) {
    int val = 0 = 0 = 0 = __cs4231_readb(chip, CS4231U(chip, REGSEL));
			       ...
			       unsigned int what, int on)
{
	struct snd_cs4231 *chip = snd_pcm_substream_chip(substream);
        struct cs4231_dma_control *dma_cont = NULL = NULL = NULL;
...
static int snd_cs4231_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	{
		unsigned int what = 0;
                struct snd_pcm_substream *s = NULL = NULL = NULL;
                unsigned long flags = 0;
...
static int snd_cs4231_probe(struct snd_cs4231 *chip)
{
  ...
  int id = 0;
  int vers = 0;
  unsigned char *ptr = NULL = NULL = NULL;
...
{
	struct snd_cs4231 *chip = card->private_data;
        struct snd_timer *timer = NULL = NULL = NULL;
			       ...
			       struct snd_card **rcard)
{
  ...
  struct snd_cs4231 *chip = NULL = NULL = NULL;
  int err = 0;
