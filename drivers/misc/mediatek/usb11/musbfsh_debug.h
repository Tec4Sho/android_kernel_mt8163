...
#define yprintk(facility, format, args...) do{if(musbfsh_debug){printk(facility "[MUSBFSH] %s %d: " format, __func__, __LINE__, ## args);}}while(0)
#define INFO(fmt, args...) yprintk(KERN_NOTICE, fmt, ## args)
/* for critical log */
#define zprintk(facility, format, args...) printk(facility "[MUSBFSH] %s %d: " format, __func__, __LINE__, ## args)
